          ...
          <div className="bg-surface-2 rounded-xl p-6 mb-6">
            ...
            <div className="flex space-x-4">
              <input
                ...
                className="flex-1 bg-surface-2 border border-border-strong rounded-lg px-4 py-3 text-white placeholder-text-3 focus:ring-2 focus:ring-gold focus:border-transparent"
              />
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex space-x-2">
                      {discussion.tags.map((tag) => (
                        <span
                          key={tag}
                          className="bg-surface-2 text-text-2 px-2 py-1 rounded text-xs"
                        >
                          #{tag}
                        </span>
                      ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {studyGroups.map((group) => (
                ...
                >
                  ...
                  <p className="text-text-2 text-sm mb-4">{group.course}</p>
                  <div className="flex justify-between items-center text-sm text-text-2">
                    <span>{group.members} members</span>
                    <span>{group.nextSession}</span>
                  </div>
                  <button className="w-full mt-4 bg-gold hover:bg-gold-dim text-white py-2 rounded-lg text-sm font-medium transition-colors">
                    Join Group
                  </button>
                </div>
              ))}
        ...
        <div className="space-y-6">
              ...
              {topContributors.map((contributor, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-surface-2 rounded-full flex items-center justify-center text-sm font-medium">
          ...
          <div className="bg-surface-2 rounded-xl p-6">
            ...
            <div className="space-y-2">
              <button className="w-full text-left bg-surface-2 hover:bg-surface-2 text-white px-4 py-3 rounded-lg text-sm transition-colors">
                Ask a Question
              </button>
              <button className="w-full text-left bg-surface-2 hover:bg-surface-2 text-white px-4 py-3 rounded-lg text-sm transition-colors">
                Create Study Group
