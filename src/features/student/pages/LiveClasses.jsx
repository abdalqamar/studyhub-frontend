    ...
    <div className="min-h-screen bg-surface text-slate-100 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-surface-2 rounded-xl p-4 text-center">
          ...
          <div className="text-text-2 text-sm">Upcoming</div>
        </div>
        <div className="bg-surface-2 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-green-400">
            {liveClasses.filter((c) => c.status === "completed").length}
          </div>
          <div className="text-text-2 text-sm">Completed</div>
        </div>
        <div className="bg-surface-2 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-purple-400">
            {liveClasses.reduce((acc, curr) => acc + curr.participants, 0)}
          </div>
          <div className="text-text-2 text-sm">Total Participants</div>
        </div>
      </div>
          ...
          >
            ...
            <div className="space-y-3 mb-4">
              <div className="flex justify-between text-sm">
                <span className="text-text-2">Instructor:</span>
                <span className="text-text-2">{classItem.instructor}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-2">Date & Time:</span>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-text-2">{classItem.duration}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-text-2">Participants:</span>
                <span className="text-text-2">
            ...
            <div className="flex space-x-2">
              ...
              {classItem.status === "completed" && classItem.recording && (
                <button className="bg-gold hover:bg-gold-dim text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex-1">
                  Watch Recording
                </button>
              )}
