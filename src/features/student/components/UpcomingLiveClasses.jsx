    ...
    <div className="bg-surface-2/50 backdrop-blur-sm rounded-xl p-6 border border-border hover:border-border-strong transition-colors h-full flex flex-col">
      ...
      <div className="space-y-3 flex-1 overflow-y-auto pr-2">
        {liveClasses.map((classItem) => (
          <div
            key={classItem.id}
            className={`bg-surface-2/80 rounded-xl border ${
              classItem.isToday
                ? "border-green-500/50 ring-1 ring-green-500/20"
                : "border-border"
            } hover:border-border-strong transition-all duration-200 p-4 group cursor-pointer`}
          >
            {/* Today Badge */}
            {classItem.isToday && (
            ...
            {/* Title */}
            <h4 className="text-white font-semibold text-sm mb-3 group-hover:text-gold transition-colors">
              {classItem.title}
            </h4>
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs text-text-2">Instructor</p>
                <p className="text-sm text-text-2 font-medium truncate">
            ...
            <div className="flex items-center gap-2 pt-3 border-t border-border">
              {classItem.isToday ? (
                ...
                <button className="flex-1 px-3 py-2 bg-surface-2 hover:bg-surface-2 text-text-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
                  <Bell className="w-4 h-4" />
                  Set Reminder
                </button>
              )}
              <button className="p-2 bg-surface-2 hover:bg-surface-2 text-text-2 rounded-lg transition-colors">
      ...
      <div className="mt-4 pt-4 border-t border-border flex-shrink-0">
        <Link
          ...
          className="w-full px-4 py-2.5 bg-surface-2 hover:bg-surface-2 text-white rounded-lg font-medium text-sm transition-colors"
        >
          View Full Schedule
        </Link>
      </div>
      ...
      {liveClasses.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-text-2 text-sm mb-2">No upcoming classes</p>
